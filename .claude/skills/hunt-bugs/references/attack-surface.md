...
| `src/insert-or-navigate-footnotes.ts` (1581 lines — the big one) | `AllReferences`, `footnoteReferenceMatches`, `referenceOccurrences` (the masked-match/raw-name iterator — BUT 3 cascade sites in this file still hand-roll the re-slice: :454, :1456, :218), `isValidFootnoteName`, `readingViewActive`, `listExistingFootnoteDefinitions`, `shouldJumpFromDefinitionToReference`, `shouldJumpFromReferenceToDefinition`, `jumpToFootnoteDefinition`, `addFootnoteSectionHeader`, `buildDefinitionAppend` (returns an optional phantom-frontmatter `prepend`), `endOfWordOffset`, `footnotePrefix`, `footnotePrefixFromEditor`, `footnotePrefixProblem`, `computeNextFootnoteNumber`, `createAutonumFootnote`, `sanitizeInlineFootnoteContent`, `inlineFootnoteSpanAt`, `navigateReferenceIfInside`, `createMatchingFootnoteDefinition`, `createFootnoteReference` (cascade steps unified 2026-08-11: `(lineText, cursorPosition, plugin, doc, …) → boolean "handled"`; creators renamed from should*) | Reference grammar, autonumbering, insertion points, inline-footnote handling, prefix parsing; shared command preamble (`withEditableEditor` — MUST stay continuation-passing: a value-returning helper adds a microtask hop that let a same-tick second press mint a second footnote, 63bc025) + caret-guard chain (`caretGuardsHandled`) |
