...
| `src/insert-or-navigate-footnotes.ts` (1283 lines — the big one) | `AllReferences`, `footnoteReferenceMatches`, `ExtractNameFromFootnote`, `isValidFootnoteName`, `readingViewActive`, `listExistingFootnoteDefinitions`, `listExistingFootnoteReferencesAndLocations`, `shouldJumpFromDefinitionToReference`, `referenceAtCursor`, `shouldJumpFromReferenceToDefinition`, `jumpToFootnoteDefinition`, `addFootnoteSectionHeader`, `buildDefinitionAppend`, `endOfWordOffset`, `footnotePrefix`, `footnotePrefixFromEditor`, `footnotePrefixProblem`, `computeNextFootnoteNumber`, `shouldCreateAutonumFootnote`, `sanitizeInlineFootnoteContent`, `inlineFootnoteSpanAt`, `inlineFootnoteExitCh`, `navigateReferenceIfInside`, `shouldCreateMatchingFootnoteDefinition`, `warnPrefilledReferenceIfInside`, `warnEmptyReferenceIfInside`, `shouldCreateFootnoteReference` | Reference grammar, autonumbering, insertion points, inline-footnote handling, prefix parsing |
| `src/linting/rules/re-index-footnotes.ts` | `reindexFootnotes`, `ReindexOptions` | Renumbering references + definitions in reading order (fixpoint loop, 20-iteration cap) |
