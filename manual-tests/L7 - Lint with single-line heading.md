...
Undo, then move the whole `# Footnotes` section (heading + definitions) to the MIDDLE of the note with prose after it, and add one stray `[^9]: nine` definition (plus a `[^9]` reference) at the very end. Run **Lint footnotes**:
