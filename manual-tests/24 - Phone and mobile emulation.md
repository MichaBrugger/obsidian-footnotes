...
- [ ] PHONE: long-press on `[^menu]` above: Obsidian's own menu opens with only its **Delete footnote and reference** item. No **Rename footnote** there, and none is possible: on a phone Obsidian builds that menu alone and gives plugins no way to add to it (2026-09-11; the desktop right-click menu is sheet 10). Dismiss it
- [ ] PHONE or emulation: rename after a long press: long-press `[^menu]` so the word is selected, dismiss the menu, then tap the **Rename footnote** toolbar icon: the dialog opens prefilled with `menu`. The plugin reads the whole selection, so it works whether the selection is the name, the whole reference, or the word around it (2026-09-11)
...
- [ ] PHONE: Rename footnote from the toolbar with a refused name: type a name that is already taken (`menu` itself, or one that fails the rules) and tap the button; the error shows and the keyboard stays up (or comes straight back) with the field focused and its text selected, so the fix can be typed at once (2026-09-11)
