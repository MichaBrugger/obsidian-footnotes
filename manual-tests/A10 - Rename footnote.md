...
- [ ] With `Per-note footnote prefix` AND the `Apply footnote prefix` lint rule ON, set this note's prefix (e.g. `p.`), rename a `[^p.1]` to the bare `5`: the modal REFUSES inline, naming the prefix to type (`p.5`) — no more silent re-prefixing by the next lint (2026-08-25). With the `Apply footnote prefix` rule OFF, the same bare rename is allowed and survives a lint
