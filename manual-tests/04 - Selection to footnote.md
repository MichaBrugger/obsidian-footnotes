# 04: selection to footnote

Automated coverage: 31 former checks now live in test/manual-selection-to-footnote.test.ts and the smoke suite; run `npm test` and `npm run test:smoke` before this sheet.

Settings: defaults. Undo between checks. Every fixture is already in this note. Tables are sheet 06 and the block zoo is sheet 05; what is left here is the handful of things only eyes and a real editor can settle.
