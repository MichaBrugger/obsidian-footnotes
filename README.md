...
- **Reindex**: renumbers footnotes `1, 2, 3…` in the order they appear and reorders their definitions to match. What happens to names follows the **Footnote names** setting: **Keep as written** (the default) leaves every name alone, **Numbered** renumbers named footnotes too, and **Named** gives numbered footnotes names taken from the first word of their definitions (`[^1]: the Smith paper` becomes `[^Smith]`), leaving the already named alone so a lint never renames twice.
