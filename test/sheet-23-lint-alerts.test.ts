...
// human to judge, so sheet 23 now says so in two lines.
//
...
describe("sheet 23: orphans, strays, and empties", () => {
    ...
    it.fails("the sheet's box: the missing-definition alert lists ALL SIX strays", () => {
        // What the code does instead: it lists TEN. The four hand-typed
        // invalid names on the "x ... y" line and the "[^c#d]" in the prose
        // have no definitions either, so they are orphaned references too
        // and the alert names them alongside the five "o" strays and
        // "[^99]". Only a name with a SPACE in it ("[^bad name]") stays out,
        // because a space stops it being a reference at all. Jason decides
        // whether the sheet or the alert should change.
        expect(alertsAfterLint()).toContain(
    ...
    it("what the code lists today: the six strays plus the four undefined invalid names", () => {
        expect(alertsAfterLint()).toContain(
            `This note has 10 footnote references with no definition ("[^99]", "[^o1]", "[^o2]", "[^o3]", "[^o4]", "[^o5]", "[^c#d]", "[^aa${BT}a]", "[^bb#b]", "[^cc${BT}c]"). Write their definitions or delete the references.`,
        );
    });
...
describe("sheet 23: a definition one blank line short", () => {
...
describe("sheet 23: invalid names", () => {
...
describe("sheet 23: code inside a definition body", () => {
...
describe("sheet 23: duplicate definitions", () => {
...
describe("sheet 23: every alert that lists footnotes", () => {
...
describe("sheet 23: definitions inside a list item", () => {
