...
describe("fixed 2026-08-10: footnote-prefix frontmatter reader keeps YAML comments", () => {
    ...
    it("strips a comment after a quoted value", () => {
        expect(footnotePrefix('---\nfootnote-prefix: "2." # chapter\n---\nbody')).toBe("2.");
    });

    it("a key with no space after the colon is not a YAML mapping entry", () => {
        // YAML: "footnote-prefix:2." is a plain scalar line, NOT a key
        const md = "---\nfootnote-prefix:2.\n---\nbody";
        expect(footnotePrefix(md)).toBe("");
    });

    it("a commented prefix must not cancel linting", () => {
        expect(
            lintBlockedByPrefix("---\nfootnote-prefix: 2. # chapter\n---\nbody[^1]"),
        ).toBeNull();
    });
