...
describe("bug: transitive-orphan chains outrun the 20-iteration reindex fixpoint cap", () => {
    ...
    it.fails("a 21-deep chain is fully deleted in ONE call (transitive promise)", () => {
        const doc = `para.\n\n${chain(21)}`;
        expect(reindexFootnotes(doc, dropOrphans)).toBe("para.\n");
    });

    it.fails("orphan deletion is idempotent on a 25-deep chain", () => {
        const doc = `para.\n\n${chain(25)}`;
